...
export const fetchWasteProvidersRequest = () => ({
  type: 'FETCH_WASTE_PROVIDERS_REQUEST'
});

// populate Waste Providers
export const populateWasteProviders = allWasteProviders => ({
  type: 'FETCH_WASTE_PROVIDERS_SUCCESS',
  response: allWasteProviders
});

// fetch Waste Provider Error
export const errorFetchWasteProviders = error => ({
  type: 'FETCH_WASTE_PROVIDERS_ERROR',
  error
});

// dispatch key and message on add or remove
export const fetchWasteProviders = () => dispatch => {
  dispatch(fetchWasteProvidersRequest());
  wasteProvidersRef
    .get()
    .then(snapshot => {
...
//       email: data.email
//     };
//     return wasteProvidersRef.doc().set(wP);
//   });
// };
